...
class Trait:
    ...
    def __init__(self, trait_name, trait_formula, trait_proficiency_formula="random.randint(1,difficulty)",
                 parent = 0, summary_trait=False, trait_unit=""):
    ...
    def generate_trait_ui(self, trait_frame, ui_row):
        ...
        ui_roll_button = Button(trait_frame, image=image_d6, height=13, width=13,
                                command=lambda self=self: roll_proficiency(self))
        ...
        ui_lookup_trait_button = Button(trait_frame, image=image_lookup, height=13, width=13,
                                        command=lambda self=self: lookup_link(self))
        ui_lookup_trait_button.grid(row=ui_row, column=5, padx=1, sticky=W)

        ui_reroll_trait_button = Button(trait_frame, image=image_reroll, height=13, width=13,
                                        command=lambda self=self: reroll_trait(self))
        ui_reroll_trait_button.grid(row=ui_row, column=6, padx=1, sticky=W)

        ui_journal_button = Button(trait_frame, image=image_journal, height=13, width=13,
                                   command=lambda: test_button("You pressed Journal"))
...
class Sol:
    ...
    def __init__(self, sol_type=0, parent="self"):
        ...
        if sol_type == 0:
            ...
            self.trait['star_diameter'] = Trait('Star Diameter',
                                                "round(" + str(self.trait['star_temp'].value) + "*0.000245,3)",
                                                trait_unit="Mkm")
            self.trait['edge_helio'] = Trait('Edge of Heliosphere',
                                             "(" + str(self.trait['star_temp'].value) + " // 115)**2", trait_unit="Mkm",
                                             summary_trait=True)
            self.trait['edge_gas_zone'] = Trait('Edge of Gas Zone', str(self.trait['edge_helio'].value) + " // 2",
                                                trait_unit="Mkm")
            self.trait['edge_hab_zone'] = Trait('Edge of Hab Zone',
                                                str(self.trait['edge_helio'].value) + "// random.randint(3,6)",
                                                trait_unit="Mkm")
            self.trait['edge_hot_zone'] = Trait('Edge of Hot Zone',
...
class Planet:
    ...
    def __init__(self, planet_type=0, orbit=0, planet_id=0, parent="self"):
        ...
        self.trait['planet_water'] = Trait("Planet water cover", "'random.choice(planet_water_type)'",
                                           summary_trait=True)
    ...
    @staticmethod
    def populate_lifeforms(region, lifeform):
        for r in region:
            ...
            for so in r.subobject:
                if so.object_type == "Lifeform":
                    ...
                    if so.trait['lifeform_type'] == "Plant-like Lifeform":
                        so.trait['regional_trait'] = Trait('Region  Trait',
                                                           "sr(random.choice(list_of['PlantFeatures']))")
                    else:
                        so.trait['regional_trait'] = Trait('Regional Trait',
                                                           "sr(random.choice(list_of['AdvLifeformUniques']))")
        return region

    # Populate Sentient lifeforms into regions
    @staticmethod
    def populate_sentients(region, sentient):
        for r in region:
...
class Region:
    ...
    def __init__(self, parent_planet=0, region_type=0, region_id=0, parent="self"):
        ...
        if self.parent_planet.trait['topography'].value == 'Rocky Topography':
            self.trait['terrain'] = Trait('Terrain', "sr(random.choice(list_of['RockyTopography']))",
                                          summary_trait=True)
        elif self.parent_planet.trait['topography'].value == 'Rocky-Water Topography':
            self.trait['terrain'] = Trait('Terrain', "sr(random.choice(list_of['RockyWaterTopography']))",
                                          summary_trait=True)
        elif self.parent_planet.trait['topography'].value == 'Water Topography':
            self.trait['terrain'] = Trait('Terrain', "sr(random.choice(list_of['WaterTopography']))",
                                          summary_trait=True)

        if self.parent_planet.trait['avg_temp'].value.split()[0] == 'Hot':
            self.trait['region_weather'] = Trait('Regional weather',
                                                 "sr(random.choice(list_of['HotRegionTemps'])) + ', ' + sr(random.choice(list_of['Wind']))",
                                                 summary_trait=True)
        elif self.parent_planet.trait['avg_temp'].value.split()[0] == 'Moderate':
            self.trait['region_weather'] = Trait('Regional weather',
                                                 "sr(random.choice(list_of['ModerateRegionTemps'])) + ', ' + sr(random.choice(list_of['Wind']))",
                                                 summary_trait=True)
        elif self.parent_planet.trait['avg_temp'].value.split()[0] == 'Cold':
            self.trait['region_weather'] = Trait('Regional weather',
                                                 "sr(random.choice(list_of['ColdRegionTemps'])) + ', ' + sr(random.choice(list_of['Wind']))",
                                                 summary_trait=True)
...
class Resource:
    ...
    def __init__(self, resource_type='', parent="self"):
        ...
        if self.resource_type == '':
            self.trait['resource_type'] = Trait("Resource Type", "random.choice(['Mineral','Mineral','Gas'])",
                                                summary_trait=True)
        elif self.resource_type != '':
            self.trait['resource_type'] = Trait(str(self.resource_type), "random.choice(['Mineral','Mineral','Gas'])",
                                                summary_trait=True)
...
class Lifeform:
    ...
    def __init__(self, lifeform_type=0, parent="self"):
        ...
        self.trait['colors'] = Trait("Lifeform Colors",
                                     "sr(random.choice(list_of['ColorCommon'])) + '/' + sr(random.choice(list_of['ColorCommon']))",
                                     summary_trait=True)
        self.trait['size'] = Trait("Lifeform Size", "sr(random.choice(list_of['AdvLifeformSize']))", summary_trait=True)
        self.trait['density'] = Trait("Lifeform Density", "sr(random.choice(list_of['AdvLifeformDensity']))",
                                      summary_trait=True)
        ...
        if self.trait['lifeform_type'].value == 'Advanced Lifeform':
            self.trait['species'] = Trait("Adv Lifeform Species", "sr(random.choice(list_of['AdvancedLifeforms']))",
                                          summary_trait=True)
            ...
            self.trait['species'] = Trait("Hybrid Species", "'Half ' + sr(random.choice(list_of['AdvancedLifeforms'])) + \
                                                    ', half ' + sr(random.choice(list_of['AdvancedLifeforms']))",
                                          summary_trait=True)
            num_uniques = max(2, random.randint(-1, 3))
            ...
            self.trait['species'] = Trait("Mythological Species", "sr(random.choice(list_of['MythologicalLifeforms']))",
                                          summary_trait=True)
            num_uniques = max(2, random.randint(-1, 3))

        elif self.trait['lifeform_type'].value == 'Advanced Aquatic Lifeform':
            self.trait['species'] = Trait("Aquatic Species", "sr(random.choice(list_of['AdvancedLifeformsAquatic']))",
                                          summary_trait=True)
            num_uniques = max(2, random.randint(-1, 3))
            ...
            self.trait['density'] = Trait("Lifeform Density", "sr(random.choice(list_of['PlantDensity']))",
                                          summary_trait=True)
            ...
            for n in range(1, max(2, random.randint(-1, 3))):
                self.trait['Trait ' + str(n)] = Trait('Species Trait ' + str(n),
                                                      "sr(random.choice(list_of['PlantFeatures']))")
...
class SentientLife:
    ...
    def __init__(self, sentient_type=0, parent="self"):
        ...
        self.trait['primary_ability_score'] = Trait("Primary ability score",
...
class Civilization:
    ...
    def __init__(self, parent="self"):
        ...
        self.trait['society_status'] = Trait("Society Status", "sr(random.choice(list_of['SocietyStatus']))",
                                             trait_unit="Society")
...
class City:
    ...
    def __init__(self, parent="self"):
        ...
        self.trait['celebrities'] = Trait("Celebrities like... ",
                                          "get_x_from_list(random.randint(2,4),'CharacterInspiration')")
        self.trait['food1'] = Trait("Popular Food Dish 1",
                                    "sr(random.choice(list_of['Food'])) + ' Cooked by ' + sr(random.choice(list_of['WaysToCook']))")
        self.trait['food2'] = Trait("Popular Food Dish 2",
                                    "sr(random.choice(list_of['Food'])) + ' Cooked by ' + sr(random.choice(list_of['WaysToCook']))")
        for t in range(1, random.randint(2, 4)):
            self.trait['transit' + str(t)] = Trait("Transit Method " + str(t),
        ...
        for t in range(1, random.randint(1, 3)):
            ...
            self.trait['sentient_' + str(t) + '_hook'] = Trait("Adventure Hook " + str(t),
                                                               "sr(random.choice(list_of['PersonStoryHook']))")
        for t in range(1, random.randint(1, 4)):
            self.trait['sentient_' + str(t)] = Trait("Point of Interest " + str(t),
                                                     "sr(random.choice(list_of['Buildings']))")
            self.trait['sentient_' + str(t) + '_hook'] = Trait("Adventure Hook " + str(t),
...
def load_ui(new_object):
    ...
    # subobjects_frame_prime = Frame(left_frame,width=0.32*screenX)
    # subobjects_frame_prime.grid(row=1,column=0,padx=2,pady=2,sticky="NESW")
    ...
    try:
        ...
        for so in curr_object.subobject:
            ...
            so_row_counter = 0
            tr_button = Button(subobjectframe, text="Expand",
                               command=lambda new_object=so, root=root: change_curr_object(new_object, root))
    ...
    canvas_scale_x = 1
    canvas_scale_y = 1
    view_canvas = Canvas(right_frame, bg="black", width=canvas_width, height=canvas_height)
    view_canvas.grid(row=0, column=0, padx=0, pady=0, sticky="NESW")
    ...
    if curr_object.object_type == "Sol":
        ...
        create_oval_by_center(view_canvas, canvas_width / 2, canvas_height / 2,
                              float(curr_object.trait['star_diameter'].value * 5 + 5), color="yellow")
        for so in curr_object.subobject:
            create_object_with_orbit(view_canvas, so, canvas_width, canvas_height, canvas_scale_x, canvas_scale_y)
